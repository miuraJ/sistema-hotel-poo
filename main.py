...
def exibir_cabecalho():
    ...
    print(f"\n{SEPARADOR}")
    print(f"Hotel POO - Sistema de Gestão")
...
def fluxo_remover_quarto(funcionario: Funcionario, hotel: Hotel):
    ...
    try:
        numero = int(input("Digite o NÚMERO do quarto para remover, ou 0 para cancelar: ").strip())
    except ValueError:
...
def fluxo_cancelar_reserva_funcionario(funcionario: Funcionario, hotel: Hotel):
    ...
    print(SEPARADOR)
    # TODO: Implementar (precisa listar reservas e escolher)
    print("Funcionalidade em desenvolvimento...")

# === FLUXOS DO HÓSPEDE ===
def mensagem_sem_hospedes():
    print(f"✗ Não há hóspedes cadastrados.")
...
def fluxo_cancelar_reserva_hospede(hospede: Hospede, hotel: Hotel):
    ...
    for i, reserva in enumerate(reservas, 1):
        ...
        print(f"{i}. Quarto #{quarto.get_numero()} - Tipo: {quarto.get_tipo()}")
    
    numero = input("Digite o número da reserva para cancelar, ou 0 para cancelar: ").strip()
